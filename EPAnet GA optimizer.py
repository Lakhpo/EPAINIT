...
def constraint_checker(
    diameter_array, min_pre, min_vel, max_vel, file, num_nodes, num_links, num_pipes, ep
):
    ...
    for pipe in range(len(diameter_array)):
        if diameter_array[pipe] > 0:
            ep.setLinkDiameter(pipe + 1, diameter_array[pipe])

    ep.saveInputFile(file)

    ep.openHydraulicAnalysis()
    ep.initializeHydraulicAnalysis()
    ...
    for node in range(num_nodes):
        ...
        if (
            ep.getNodePressure(node + 1) < min_pre
            and ep.getNodeType(node + 1) == "JUNCTION"
        ):
    ...
    for pipe in range(num_pipes):
        ...
        if ep.getLinkVelocity(pipe + 1) < min_vel:
            fail_min_vel_list.append(pipe)
            fail_min_vel = True
        elif ep.getLinkVelocity(pipe + 1) > max_vel:
...
def population_cost(
    ...
    for diameter_config in range(population_size):
        ...
        config_costs[diameter_config] = fitness_function(
            network_cost(length_array, current_config),
            constraint_checker(
                current_config,
                min_pre,
                min_vel,
...
def mutate_config(evolved_generation, mutations=1):
    for pipe_config in range(evolved_generation.shape[0]):
        for mutation in range(mutations):
            evolved_generation[
                pipe_config, np.random.randint(0, evolved_generation.shape[1])
...
def main():
    ...
    for pipe in range(num_pipes):
        diameter_array[pipe] = d.getLinkDiameter(pipe + 1)
        length_array[pipe] = d.getLinkLength(pipe + 1)
    ...
    for ga_generation in range(num_generations):
        ...
        best_configs = get_best_configs(
            population_array, config_costs, num_configs_to_merge
        )
        ...
        # VERSION 18: if best cost is inf (means every network is infeasible), then increase all pipe by 1 size
        # the old genetic algorithm in ga version 17 got stumped by the Hanoi network and fails to converge
        # this is because all the configs were inf - hence no comparison and selection and improvement could be made
        if np.min(least_cost_configs) == float("inf"):
            with np.nditer(population_array, op_flags=["readwrite"]) as iterator:
                for pipe in iterator:
                    # print("Pipe size: ", pipe)
                    pipe_index = np.where(commercial_diameters == pipe)[0][0]
                    # print("Pipe index: ", pipe_index)
                    if pipe < np.max(commercial_diameters):
                        pipe[...] = commercial_diameters[pipe_index + 1]
                        # print("Size increased to: ", pipe)
    ...
    for pipe in range(len(least_cost_network)):
        d.setLinkDiameter(pipe + 1, least_cost_network[pipe])

    d.saveInputFile(inp_file)
    d.closeNetwork()
    ...
    if final_costs[least_cost_index] == float("inf"):
        print(
            "No solutions found. Problem may be overconstrained or insufficient number of generations."
        )
        sg.Popup(
            "No solutions found. Problem may be overconstrained or insufficient number of generations."
        )
...
layout = [
    [
        sg.Text("EPANET network .inp file:", size=(21, 1)),
        sg.Input(
            key="inp_filepath",
            default_text="C:/Users/FanBo/PycharmProjects/fyp_ga/networks(pipeSizing)/BIN/BIN.inp",
        ),
    ...
    [
        ...
        sg.Input(
            key="csv_filepath",
            default_text="C:/Users/FanBo/PycharmProjects/fyp_ga/BIN.csv",
        ),
        sg.FileBrowse(file_types=(("CSV", ".csv"),)),
    ],
    [
        sg.Text("Minimum nodal pressure (m):", size=(21, 1)),
        sg.Input(key="min_pressure", size=(10, 1), default_text=30),
    ],
    [
        sg.Text("Minimum pipe velocity (m/s):", size=(21, 1)),
        sg.Input(key="min_velocity", size=(10, 1), default_text=0),
    ],
    [
        sg.Text("Maximum pipe velocity (m/s):", size=(21, 1)),
        sg.Input(key="max_velocity", size=(10, 1), default_text="inf"),
        sg.Text("", size=(7, 1)),
        sg.Button("Run Genetic Algorithm", size=(20, 1)),
    ],
    [
        sg.Text("Population size:", size=(21, 1)),
        sg.Input(key="pop_size", size=(10, 1), default_text=12),
    ],
    [
        sg.Text("Number of generations:", size=(21, 1)),
        sg.Input(key="num_generations", size=(10, 1), default_text=500),
    ],
]

window = sg.Window(
    "Pipeline Cost Optimisation - UCL Third Year Project - Bowen Fan", layout
)
